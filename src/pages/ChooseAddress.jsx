...
export default (props) => {
  ...
  useEffect(() => {
    Axios.get(`${BASE_URL}/address/`, {
      ...
      .catch((error) => {
        console.log("Can't load address.");
      });
  }, []);
      ...
      <React.Fragment>
        <Container maxWidth={"sm"} className={classes.root}>
          ...
          <Paper className={classes.paper} elevation={3}>
            <List
              subheader={
                <ListSubheader disableSticky> Addresses </ListSubheader>
              }
            >
              {addresses.map((address, index) => (
                <React.Fragment>
                  <ListItem key={index}>
                    <ListItemText
                      primary={address.landmark || "No landmark"}
                      secondary={
                        <React.Fragment>
                    ...
                    <ListItemSecondaryAction>
                      <Radio
                        edge={"end"}
                        checked={ctx.state.address_id === address.id}
                        onChange={() => {
                          ctx.dispatch({
